...
from multilingual_data_collator import MultilingualSelfDistillationDataCollator
...
class OPSDTrainer(SFTTrainer):
    ...
    def __init__(
        ...
        include_problem_en: bool = True,
        include_reference_solution_en: bool = True
    ):
        ...
        if data_collator is None:
            data_collator = MultilingualSelfDistillationDataCollator(
                tokenizer=processing_class, 
                max_length=args.max_length, 
                reason_first=reason_first, 
                student_enable_thinking=student_enable_thinking, 
                include_problem_en=include_problem_en, 
                include_reference_solution_en=include_reference_solution_en
            )
        ...
        if reason_first and not include_reference_solution_en:
            raise ValueError(
                "reason_first=True requires include_reference_solution_en=True, "
                "because reason_first_prompt asks the teacher to analyze the English reference solution."
            )
    ...
    def training_step(
        ...
        teacher_prompt_texts = self.processing_class.batch_decode(
            teacher_prompts,
            skip_special_tokens=False,
        )
        teacher_completion_texts = self.processing_class.batch_decode(
            generation_ids,
            skip_special_tokens=False,
        )

        # Collect generation outputs for saving
        for prompt, completion, teacher_prompt, teacher_completion in zip(
            prompt_texts,
            completion_texts,
            ...
            self._generation_outputs_buffer.append(
                {
                    ...
                    "teacher_evaluated_completion": teacher_completion,
                }
            )
